...
foreach($student as $columns){
    ?>  <tr class="<?php echo $tr_class[$i % 3] ?>">
        ...
        <?php if(get_cookie('type') === 'admin'){?>
            <td><a href = "<?=base_url()?>index.php/student/delete_student/<?php echo $columns['id'] ?>"><?=$delete?></span></a></td>
            <td><a href = '<?=base_url(); ?>index.php/student/update/<?=$columns['id'];?>' ><?=$update?></a></td>
        <?php } ?>
    </tr> <?php
    $i++;
}
